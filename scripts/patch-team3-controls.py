...
Path('src/battles').mkdir(parents=True, exist_ok=True)
Path('src/battles/teamSelection.ts').write_text(r'''export type TeamSelectionSortMode = 'value' | 'hp' | 'attack' | 'defense' | 'speed' | 'name';
...
old_load = """  const loadCards = useCallback(async (query = '') => {
    if (!battleId) return;
    try {
...
"""
new_load = """  const loadCards = useCallback(async () => {
    if (!battleId) return;
...
"""
replace(p, old_effect, new_effect)

anchor = """  }, [deadline, state?.turn, state?.status]);

  const toggleCard = (card: TeamBattleCard) => {
"""
insertion = """  }, [deadline, state?.turn, state?.status]);
...
old_forfeit = """  const confirmForfeit = () => {
...
"""
new_forfeit = """  const confirmForfeit = () => {
...
"""
replace(p, old_forfeit, new_forfeit)
...
draft_insert = """              <>
                ...
                <View style={styles.selectedRow}>
"""
replace(p, draft_anchor, draft_insert)

confirm_team_block = """                <Pressable disabled={working || selected.length !== 3} onPress={() => void lockTeam()} style={[styles.primaryButton, { backgroundColor: selected.length === 3 ? colors.accent : colors.border, opacity: working ? 0.65 : 1 }]}>
...
"""
confirm_team_new = """                <Pressable disabled={working || selected.length !== 3} onPress={() => void lockTeam()} style={[styles.primaryButton, { backgroundColor: selected.length === 3 ? colors.accent : colors.border, opacity: working ? 0.65 : 1 }]}>
                  {working ? <ActivityIndicator color="#fff" /> : <Ionicons name="checkmark-circle" size={20} color="#fff" />}
                  <Text style={styles.primaryButtonText}>Confirmar equipe 3×3</Text>
                </Pressable>
              </>
...
"""
replace(p, confirm_team_block, confirm_team_new)

modal_header_anchor = """              <Pressable onPress={() => setPickerOpen(false)} style={[styles.pickerClose, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <TextInput
"""
modal_header_new = """              <Pressable onPress={() => setPickerOpen(false)} style={[styles.pickerClose, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Ionicons name="close" size={20} color={colors.text} />
              </Pressable>
            </View>
            ...
            <TextInput
"""
replace(p, modal_header_anchor, modal_header_new)
replace(p, "              data={cards}\n", "              data={cardsLoading ? [] : visibleCards}\n")
replace(
    p,
...
"""
replace(p, modal_end, extra_modals)
...
Path('scripts/team3-controls-audit.mjs').write_text(r'''import fs from 'node:fs';
...
'''
Path('supabase/migrations/20260904233403_team3_forfeit_respects_team_selection.sql').write_text(migration)

pkg = Path('package.json')
text = pkg.read_text()
old = 'node scripts/admin-card-grant-audit.mjs"'
new = 'node scripts/admin-card-grant-audit.mjs && node scripts/team3-controls-audit.mjs"'
if old not in text:
