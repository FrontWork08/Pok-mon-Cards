    ...
    <Screen title="Auto Booster" subtitle="Escolha o booster, a quantidade e veja o custo total antes de abrir.">
      ...
      <View style={[styles.panel,{backgroundColor:colors.surface,borderColor:colors.border}]}>
        ...
        <View style={styles.packGrid}>
          {packs.map((pack)=>{const active=selectedPackId===pack.id;return <Pressable key={pack.id} onPress={()=>setSelectedPackId(pack.id)} style={[styles.packCard,{backgroundColor:active?colors.accentSoft:colors.surfaceAlt,borderColor:active?colors.accent:colors.border}]}>
            {pack.booster_art_url?<Image source={{uri:pack.booster_art_url}} style={styles.packImage} resizeMode="contain"/>:<View style={[styles.packImage,{backgroundColor:colors.bg}]}/>}<View style={{flex:1,minWidth:0}}><Text numberOfLines={2} style={[styles.packName,{color:colors.text}]}>{pack.name}</Text><Text style={[styles.packPrice,{color:active?colors.yellow:colors.muted}]}>{pack.currency==='diamonds'?'💎':'🪙'} {pack.price.toLocaleString('pt-BR')}</Text></View>{active?<Ionicons name="checkmark-circle" size={20} color={colors.accent}/>:null}
          </Pressable>;})}
        </View>
      </View>
