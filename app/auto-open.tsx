  ...
  return (
    <Screen title="Auto Booster" subtitle="Escolha o booster, a quantidade e veja o custo total antes de abrir.">
      ...
      <View style={[styles.panel,{backgroundColor:colors.surface,borderColor:colors.border}]}>
        ...
        {!perks?.autoOpenGamepass?<Pressable onPress={()=>router.push('/gamepasses')} style={[styles.storeLink,{borderColor:colors.accent}]}><Ionicons name="storefront" size={17} color={colors.accent}/><Text style={[styles.storeLinkText,{color:colors.accent}]}>VER TODAS AS GAMEPASSES</Text></Pressable>:null}
      </View>
      ...
      {summaryCards.length?<>
        <View style={styles.summaryHeader}>
          <Text style={[styles.sectionTitle,{color:colors.text}]}>Resumo das cartas</Text>
          <View style={[styles.inspectHint,{backgroundColor:colors.accentSoft,borderColor:colors.accent}]}><Ionicons name="eye-outline" size={14} color={colors.accent}/><Text style={[styles.inspectHintText,{color:colors.accent}]}>TOQUE PARA INSPECIONAR</Text></View>
        </View>
        <View style={styles.cardGrid}>{summaryCards.map(({card,quantity:count})=><Pressable
          key={card.id}
        ...
        >
          ...
          <View style={{flex:1,minWidth:0}}>
            <Text numberOfLines={2} style={[styles.cardName,{color:colors.text}]}>{card.name}</Text>
            <Text style={[styles.cardMeta,{color:colors.muted}]}>x{count} • {card.rarity??'Sem raridade'}</Text>
            {Number(card.marketPriceUsd)>0?<Text style={[styles.cardPrice,{color:colors.yellow}]}>US$ {Number(card.marketPriceUsd).toFixed(2)}</Text>:null}
            <View style={[styles.inspectBadge,{backgroundColor:colors.accentSoft}]}><Ionicons name="eye-outline" size={12} color={colors.accent}/><Text style={[styles.inspectBadgeText,{color:colors.accent}]}>INSPECIONAR</Text></View>
          </View>
