...
export default function SetsScreen() {
  ...
  return (
    <Screen title="Coleções por Set" subtitle="Acompanhe quantos cards você já conseguiu em cada coleção.">
      ...
      <View style={styles.searchBox}><Ionicons name="search" size={19} color="#7186A3" /><TextInput value={search} onChangeText={setSearch} placeholder="Buscar set..." placeholderTextColor="#71809A" style={styles.search} /></View>
        ...
        {filtered.map((set) => {
          ...
          return (
            <Pressable key={set.set_id} style={[styles.setCard, { width: cardWidth as any }]} onPress={() => router.push(`/set/${set.set_id}`)}>
              <View style={styles.setImageWrap}>{set.representative_image ? <Image source={{ uri: set.representative_image }} style={styles.setImage} resizeMode="contain" /> : <Ionicons name="albums-outline" size={42} color="#607B9E" />}</View>
              ...
              <Ionicons name="chevron-forward" size={18} color="#647B98" />
            </Pressable>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  hero: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 18, borderRadius: 23, backgroundColor: '#10284B', borderWidth: 1, borderColor: '#285A9A' },
  ...
  heroText: { color: '#9EB2CE', fontSize: 10 },
  heroRight: { alignItems: 'flex-end' },
  completeValue: { color: '#65D894', fontSize: 27, fontWeight: '900' },
  completeLabel: { color: '#91A6C1', fontSize: 9 },
  searchBox: { height: 50, flexDirection: 'row', alignItems: 'center', gap: 9, paddingHorizontal: 13, borderRadius: 15, backgroundColor: '#101D30', borderWidth: 1, borderColor: '#263E5C' },
  search: { flex: 1, height: '100%', color: '#fff', fontSize: 13 },
  sectionRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  sectionTitle: { color: '#fff', fontSize: 20, fontWeight: '900' },
  count: { color: '#8297B2', fontSize: 11, fontWeight: '800' },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  setCard: { minHeight: 120, flexDirection: 'row', alignItems: 'center', gap: 11, padding: 10, borderRadius: 18, backgroundColor: '#101D30', borderWidth: 1, borderColor: '#263E5C' },
  setImageWrap: { width: 72, height: 99, borderRadius: 12, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', backgroundColor: '#091524' },
  ...
  setId: { color: '#6F85A3', fontSize: 8, fontWeight: '800', marginTop: 2 },
  percent: { color: gameTheme.colors.yellow, fontSize: 13, fontWeight: '900' },
  progressText: { color: '#91A6BF', fontSize: 9, marginTop: 9 },
  track: { height: 6, borderRadius: 999, overflow: 'hidden', backgroundColor: '#1B2A3E', marginTop: 5 },
