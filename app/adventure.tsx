...
type ModeCardProps = {
  ...
  disabled?: boolean;
  busy?: boolean;
  blockedByOtherAction?: boolean;
  onPress: () => void;
  button?: string;
};

function friendlyAdventureError(error: unknown) {
  const raw = String(error instanceof Error ? error.message : error ?? '');
  if (raw.includes('ACTIVE_BATTLE_EXISTS')) return 'Você já tem outra batalha em andamento. Termine ou desista dela antes de iniciar um modo da aventura.';
  if (raw.includes('TEAM_NEEDS_3_GAME_CARDS')) return 'Você precisa ter pelo menos 3 Pokémon válidos para batalha na Bag.';
  if (raw.includes('ROGUE_NEEDS_3_GAME_CARDS')) return 'O Rogue Draft precisa de pelo menos 3 Pokémon válidos na sua coleção.';
  if (raw.includes('GUILD_REQUIRED')) return 'Entre em uma guilda para participar da Raid de Guilda.';
  if (raw.includes('RAID_FINISHED')) return 'Esta raid já terminou. Atualize a tela para carregar o próximo boss.';
  if (raw.includes('RAID_DAILY_LIMIT_REACHED')) return 'Você atingiu o limite diário de tentativas desta raid.';
  if (raw.includes('ELITE_ALREADY_COMPLETED_THIS_WEEK')) return 'A Elite Four desta semana já foi concluída.';
  if (raw.includes('ADVENTURE_BOT_UNAVAILABLE')) return 'Nenhum adversário compatível está disponível agora. Tente novamente em instantes.';
  if (raw.includes('ADVENTURE_TEAM_UNAVAILABLE')) return 'Não foi possível montar a equipe adversária deste estágio. Atualize e tente novamente.';
  return raw || 'Não foi possível iniciar esta batalha.';
}
...
export default function AdventureHubScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setNotice(null);
      setHub(await getAdventureHub());
    } catch (e) {
      setNotice(e instanceof Error ? e.message : 'Não foi possível abrir a Jornada Trainer.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function launch(kind: AdventureKind, refId?: string | null) {
    if (working) return;
    const actionKey = `${kind}:${refId ?? ''}`;
    try {
      setWorking(actionKey);
      setNotice(null);
      const result = await startAdventureBattle(kind, refId);
      const battleId = String(result?.battleId ?? '').trim();
      if (!battleId) throw new Error('BATTLE_ROUTE_MISSING');
      router.push({ pathname: '/team-battle/[id]', params: { id: battleId } } as any);
    } catch (e) {
      const message = friendlyAdventureError(e);
      setNotice(message);
      Alert.alert('Trainer Adventure', message);
    } finally {
      setWorking(null);
    }
  }

  const isBusy = useCallback((kind: AdventureKind, refId?: string | null) => working === `${kind}:${refId ?? ''}`, [working]);
  const kantoPct = hub?.kanto.total ? Math.round(hub.kanto.completed / hub.kanto.total * 100) : 0;
  const raidPct = hub?.raid.hasGuild && hub.raid.maxHp
    ? Math.max(0, Math.min(100, Math.round((1 - (Number(hub.raid.currentHp ?? 0) / Math.max(1, Number(hub.raid.maxHp)))) * 100)))
    : 0;
  ...
  return <Screen title="Trainer Adventure" subtitle="Jornada, Battle Tower, Elite Four, Raids, Roguelike e desafios usando o mesmo motor Game Boy 3×3.">
    ...
    {hub ? <>
      ...
      <View style={styles.grid}>
        <ModeCard icon="business" title="Battle Tower" subtitle="Suba até onde conseguir. Chefes a cada 5 andares." meta={`Andar ${hub.tower.floor} • recorde ${hub.tower.bestFloor}`} accent="#8DD7FF" onPress={() => void launch('tower')} busy={isBusy('tower')} blockedByOtherAction={!!working && !isBusy('tower')} button="SUBIR"/>
        <ModeCard icon="shield-checkmark" title="Elite Four semanal" subtitle="Cinco estágios que mudam de dificuldade ao longo da Liga." meta={hub.elite.completed ? 'CONCLUÍDO ESTA SEMANA' : `Estágio ${hub.elite.stage}/5 • ${hub.elite.wins} vitórias`} accent="#D4A6FF" onPress={() => void launch('elite')} busy={isBusy('elite')} blockedByOtherAction={!!working && !isBusy('elite')} disabled={hub.elite.completed} button={hub.elite.completed ? 'CONCLUÍDO' : 'DESAFIAR'}/>
        <ModeCard icon="people" title="Raid de Guilda" subtitle={hub.raid.hasGuild ? `${hub.raid.name ?? 'Boss'} • ${String(hub.raid.type ?? '').toUpperCase()}` : 'Entre em uma guilda para liberar o boss coletivo.'} meta={hub.raid.hasGuild ? `${raidPct}% derrotado • seu dano ${Number(hub.raid.myDamage ?? 0).toLocaleString('pt-BR')}` : 'GUILDA NECESSÁRIA'} accent="#65D894" onPress={() => void launch('raid')} busy={isBusy('raid')} blockedByOtherAction={!!working && !isBusy('raid')} disabled={!hub.raid.hasGuild || hub.raid.defeated} button={hub.raid.defeated ? 'DERROTADO' : 'ATACAR'}/>
        <ModeCard icon="dice" title="Rogue Draft Run" subtitle="Pool limitado da sua coleção, novos Pokémon ao avançar e fim da run ao perder." meta={hub.rogue.active ? `Andar ${hub.rogue.floor} • pool ${hub.rogue.cards.length}` : 'NOVA RUN'} accent="#FFB16A" onPress={() => void launch('rogue')} busy={isBusy('rogue')} blockedByOtherAction={!!working && !isBusy('rogue')} button={hub.rogue.active ? 'CONTINUAR' : 'COMEÇAR'}/>
      </View>
...
function ModeCard({ icon, title, subtitle, meta, accent, disabled, busy, blockedByOtherAction, onPress, button = 'JOGAR' }: ModeCardProps) {
  const blocked = Boolean(disabled || busy || blockedByOtherAction);
  return <View style={[styles.modeCard, { borderColor: accent + '77' }]}><View style={[styles.modeIcon, { backgroundColor: accent + '18' }]}><Ionicons name={icon} size={25} color={accent}/></View><Text style={styles.staticTitle}>{title}</Text><Text style={styles.staticSub}>{subtitle}</Text><Text style={[styles.modeMeta, { color: accent }]}>{meta}</Text><Pressable disabled={blocked} onPress={onPress} style={({ pressed }) => [styles.modeButton, { backgroundColor: disabled ? '#34404C' : accent, opacity: blockedByOtherAction ? .55 : pressed ? .78 : 1 }]}>{busy ? <View style={styles.buttonBusy}><ActivityIndicator size="small" color="#07111F"/><Text style={[styles.modeButtonText, { color: '#07111F' }]}>ABRINDO...</Text></View> : <Text style={[styles.modeButtonText, { color: disabled ? '#8B98A5' : '#07111F' }]}>{button}</Text>}</Pressable></View>;
}

function QuickLink({ icon, label, meta, onPress }: { icon: keyof typeof Ionicons.glyphMap; label: string; meta: string; onPress: () => void }) {
  return <Pressable onPress={onPress} style={styles.quick}><Ionicons name={icon} size={20} color="#8DD7FF"/><View style={{ flex: 1 }}><Text style={styles.quickLabel}>{label}</Text><Text style={styles.quickMeta}>{meta}</Text></View><Ionicons name="chevron-forward" size={16} color="#6F8496"/></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 12, fontWeight: '900', letterSpacing: .9, marginTop: 3 }, grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, modeCard: { width: '48%', minWidth: 150, borderWidth: 1, borderRadius: 16, padding: 13, backgroundColor: '#0D1822', gap: 7 }, staticTitle: { color: '#EEF5FA', fontSize: 14, fontWeight: '900' }, staticSub: { color: '#8295A4', fontSize: 10, lineHeight: 14, minHeight: 42 }, modeMeta: { fontSize: 10, fontWeight: '800', minHeight: 28 }, modeButton: { borderRadius: 9, paddingVertical: 9, alignItems: 'center', minHeight: 36, justifyContent: 'center' }, modeButtonText: { fontWeight: '900', fontSize: 10 }, buttonBusy: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 },
