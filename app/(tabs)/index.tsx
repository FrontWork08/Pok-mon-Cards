...
import { getHomeContinueItems, getHomeDashboard, type HomeContinueItem } from '@/services/home';
...
export default function HomeScreen() {
  ...
  const [profile,setProfile]=useState<any>(null);const[stats,setStats]=useState({totalCards:0,species:0,completedTrades:0});const[battlePass,setBattlePass]=useState<BattlePassState|null>(null);const[continueItems,setContinueItems]=useState<HomeContinueItem[]>([]);const[loading,setLoading]=useState(true);const[claiming,setClaiming]=useState(false);const[notice,setNotice]=useState<string|null>(null);const loadedOnce=useRef(false);
  const load=useCallback(async()=>{try{if(!loadedOnce.current)setLoading(true);const[dashboard,pass,continueData]=await Promise.all([getHomeDashboard(),getBattlePass().catch(()=>null),getHomeContinueItems().catch(()=>[])]);setProfile(dashboard.profile);setStats(dashboard.stats);setBattlePass(pass);setContinueItems(continueData);loadedOnce.current=true;}finally{setLoading(false);}},[]);
