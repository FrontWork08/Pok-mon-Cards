...
export default function PacksScreen() {
  ...
  const packWidth = width < 560 ? '100%' : width >= 1100 ? '32.4%' : '49%';
  const artWidth = width < 560 ? 188 : width >= 1100 ? 162 : 174;
  ...
  return <Screen title="Pack Shop" subtitle="Escolha um booster 2D, rasgue o lacre e revele seus cards.">
    ...
    <View style={styles.packGrid}>{visiblePacks.map((pack) => {
      ...
      return <Pressable key={pack.id} onPress={() => choosePack(pack)} style={[styles.pack, { width: packWidth as any, backgroundColor: colors.surface, borderColor: colors.border }, !affordable && styles.packUnaffordable]}>
        <View style={[styles.displayCase, { backgroundColor: colors.surfaceAlt }]}><View style={[styles.spotlight, { backgroundColor: colors.accent }]} /><BoosterPack2D pack={pack} width={artWidth} /><View style={styles.shelfShadow} /><View style={[styles.cardCountBadge, { backgroundColor: colors.bg }]}><Text style={[styles.cardCountText, { color: colors.text }]}>{pack.cards_per_pack} cards</Text></View></View>
...
const styles = StyleSheet.create({
  ...
  packGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, pack: { borderRadius: 20, padding: 9, borderWidth: 1 }, packUnaffordable: { opacity: .72 }, displayCase: { height: 315, borderRadius: 16, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }, spotlight: { position: 'absolute', top: -80, width: 230, height: 260, borderRadius: 130, opacity: .14, transform: [{ scaleX: 1.45 }] }, shelfShadow: { position: 'absolute', bottom: 21, width: 155, height: 22, borderRadius: 80, backgroundColor: 'rgba(0,0,0,.56)', transform: [{ scaleX: 1.12 }] }, cardCountBadge: { position: 'absolute', top: 9, right: 9, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 999, opacity: .9 }, cardCountText: { fontSize: 8, fontWeight: '900' }, packTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 9 }, packName: { fontSize: 14, lineHeight: 18, fontWeight: '900', minHeight: 36 }, setId: { fontSize: 9, fontWeight: '800', marginTop: 2 }, packFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 8 }, price: { fontSize: 13, fontWeight: '900' }, openButton: { paddingHorizontal: 11, paddingVertical: 8, borderRadius: 11, borderWidth: 1 }, openButtonText: { fontSize: 9, fontWeight: '900', letterSpacing: .3 },
