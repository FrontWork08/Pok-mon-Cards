...
export default function BagScreen() {
  ...
  const loadBag = useCallback(async () => {
    try { setLoading(true); setCards(await getMyBag()); }
    catch { setCards([]); }
  ...
  const collectionValue = useMemo(() => cards.reduce((sum, entry) => sum + Number(entry.cards?.game_value ?? 0) * Number(entry.quantity ?? 0), 0), [cards]);
  const mostValuable = useMemo(() => [...cards].filter((entry) => entry.cards).sort((a, b) => Number(b.cards?.game_value ?? 0) - Number(a.cards?.game_value ?? 0))[0] ?? null, [cards]);
  ...
  const visibleCards = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (sortMode === 'value') return Number(b.cards?.game_value ?? 0) - Number(a.cards?.game_value ?? 0);
