...
export default function BagScreen() {
  ...
  const refreshPrices = useCallback(async (entries: OwnedCardEntry[], force = false) => {
    ...
    try {
      ...
      // Prices are optional market metadata; the Bag remains usable if refresh fails.
    } finally {
  ...
  const loadBag = useCallback(async () => {
    try {
      ...
      setCards(entries);
      const hasMissingPrice = entries.some((entry) => entry.cards?.market_price_updated_at == null);
      if (hasMissingPrice) refreshPrices(entries, false);
    } catch {
      setCards([]);
    } finally {
      setLoading(false);
