...
<View style={styles.top}><View style={{flex:1,minWidth:220}}><Text style={[styles.eyebrow,{color:colors.yellow}]}>TRAINER MARKET</Text><Text style={[styles.pageTitle,{color:colors.text}]}>Mercado de Treinadores</Text><Text style={[styles.subtitle,{color:colors.muted}]}>Lojas ao vivo, cartas em custódia segura e pagamento em Coins.</Text></View></View>
    ...
    <View style={[styles.shopPanel,{backgroundColor:colors.surface,borderColor:colors.accent}]}>
      ...
      {(hub?.ownedShopThemes.length??0)>0?<Text style={[styles.sectionHint,{color:colors.muted}]}>Temas premium comprados na Economy 2.0 aparecem aqui automaticamente.</Text>:null}
...
function ListingCard({item,myId,working,onBuy,onOffer}:{item:MarketplaceListing;myId:string;working:boolean;onBuy:(item:MarketplaceListing)=>void;onOffer:(item:MarketplaceListing)=>void}){
  ...
  return <View style={[styles.listing,{backgroundColor:colors.surface,borderColor:listingBoosted?colors.yellow:themeColor}]}>
