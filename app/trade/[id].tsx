  ...
  return (
    <View style={[styles.offerPanel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      ...
      {cards.length === 0 ? <Text style={[styles.offerEmpty, { color: colors.muted }]}>{ownerEmpty}</Text> : cards.map((item) => {
        const relation = item.cards;
        const card = Array.isArray(relation) ? relation[0] : relation;
        return <Pressable key={`${item.owner_id}-${item.card_id}`} onPress={()=>card?.id&&router.push(('/card/'+card.id) as never)} style={[styles.offerItem, { backgroundColor: colors.surfaceAlt }]}>
          {card?.image_small ? <Image source={{ uri: card.image_small }} style={styles.offerThumb} resizeMode="contain" /> : <View style={styles.offerThumb} />}
          <View style={{ flex: 1 }}><Text style={[styles.offerName, { color: colors.text }]}>{card?.pokemon_name ?? item.card_id}</Text><Text style={[styles.offerMeta, { color: colors.muted }]}>{card?.rarity ?? 'Sem raridade'} • ×{item.quantity}</Text><Text style={[styles.offerOpenHint,{color:colors.accent}]}>ABRIR CARD DETAIL</Text></View>
          <Text style={[styles.offerValue, { color: colors.yellow }]}>{card?.market_price_usd != null ? formatUsd(Number(card.market_price_usd) * Number(item.quantity ?? 0)) : 'US$ —'}</Text>
...
const styles = StyleSheet.create({
  ...
  offersGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, offerPanel: { flexGrow: 1, flexBasis: 300, minWidth: 270, borderRadius: 19, borderWidth: 1, padding: 13, gap: 8 }, offerTitle: { fontSize: 14, fontWeight: '900' }, offerEmpty: { fontSize: 10, lineHeight: 15, paddingVertical: 14 }, offerItem: { flexDirection: 'row', alignItems: 'center', gap: 9, borderRadius: 12, padding: 7 }, offerThumb: { width: 43, height: 58, borderRadius: 5 }, offerName: { fontSize: 11, fontWeight: '900' }, offerOpenHint:{fontSize:6.2,fontWeight:'900',letterSpacing:.3,marginTop:3}, offerMeta: { fontSize: 8, marginTop: 2 }, offerValue: { fontSize: 10, fontWeight: '900' },
