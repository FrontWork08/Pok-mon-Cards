...
export default function BattleScreen() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    try {
      const [{ data: auth }, battleData, roundData, eventData, stakeData, bagData, deckData] = await Promise.all([
  ...
  const pickerBag = useMemo(() => allowedIds ? bag.filter((entry) => entry.cards?.id && allowedIds.has(entry.cards.id)) : bag, [allowedIds, bag]);
  ...
  useEffect(() => {
    if (selectedId && allowedIds && !allowedIds.has(selectedId)) setSelectedId(null);
  ...
  return (
    <View style={[styles.safe, { backgroundColor: colors.bg }]}>
      <Stack.Screen options={{ headerShown: true, title: battle.mode === 'mystery' ? 'Mystery Battle' : 'Quick Battle', headerStyle: { backgroundColor: colors.bg }, headerTintColor: colors.text }} />
      <ScrollView contentContainerStyle={[styles.content, selecting && styles.contentWithDock]} showsVerticalScrollIndicator={false}>
        ...
        {invited ? (
          <View style={[styles.panel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.panelText, { color: colors.muted }]}>{amChallenger ? 'Aguardando o outro treinador aceitar.' : `@${challenger?.username ?? 'Treinador'} quer uma ${battle.mode === 'mystery' ? 'Mystery Battle melhor de 3' : 'Quick Battle'}.`}</Text>
          ...
          <>
            ...
            {!selfLocked ? (
              <View style={[styles.sourcePanel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <View style={styles.sourceLabelRow}><Text style={[styles.sourceLabel, { color: colors.muted }]}>FONTE DAS CARTAS</Text><Pressable onPress={() => router.push('/decks')}><Text style={[styles.editDecks, { color: colors.accent }]}>EDITAR DECKS</Text></Pressable></View>
        ...
        {completed ? (
          <View style={[styles.resultPanel, { backgroundColor: colors.surface, borderColor: colors.yellow }]}>
            ...
            {battle.stake_type === 'card' ? <View style={[styles.stakeResult, { backgroundColor: colors.surfaceAlt }]}><Text style={[styles.stakeResultText, { color: colors.text }]}>🎴 O vencedor recebeu as cartas mantidas em escrow.</Text>{myStake?.cards ? <Text style={[styles.stakeResultValue, { color: colors.yellow }]}>Valor de mercado da sua aposta: {(() => { const stakeCard = Array.isArray(myStake.cards) ? myStake.cards[0] : myStake.cards; return stakeCard?.market_price_usd != null ? formatUsd(Number(stakeCard.market_price_usd)) : 'US$ —'; })()}</Text> : null}</View> : null}
      ...
      {selecting ? (
        <View style={[styles.actionDock, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
          ...
          <Pressable style={[styles.lockDock, { backgroundColor: selfLocked ? '#274E3B' : colors.yellow }, (!selectedId || working || selfLocked || remaining === 0) && styles.disabled]} onPress={lock} disabled={!selectedId || working || selfLocked || remaining === 0}>
            ...
            <View><Text style={[styles.dockSmall, { color: selfLocked ? '#9CCDB1' : '#564912' }]}>{selectedEntry?.cards ? (selectedEntry.cards.market_price_usd != null ? formatUsd(Number(selectedEntry.cards.market_price_usd)) : 'US$ —') : 'SELEÇÃO SECRETA'}</Text><Text style={[styles.lockDockText, { color: selfLocked ? '#E2FFEE' : '#07111F' }]}>{selfLocked ? 'CARTA TRAVADA' : remaining === 0 ? 'RESOLVENDO…' : 'TRAVAR CARTA'}</Text></View>
          </Pressable>
        </View>
      ) : null}
      ...
      <CardPickerModal
        ...
        title="Carta de batalha"
        subtitle={sourceDeck === 'bag' ? 'Sua Bag inteira • o oponente não verá sua escolha.' : `Deck: ${selectedDeck?.name ?? 'selecionado'} • escolha em segredo.`}
        bag={pickerBag}
        mode="single"
...
const styles = StyleSheet.create({
  ...
  timerPanel: { alignItems: 'center', paddingVertical: 10 }, roundLabel: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3 }, timer: { fontSize: 43, fontWeight: '900', marginTop: 4 }, timerHint: { fontSize: 9, textAlign: 'center', lineHeight: 14, marginTop: 5 },
