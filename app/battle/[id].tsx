  ...
  : gameStyleBattle ? `${pickerBag.length} cartas • escolha pela espécie/forma; sem Energia TCG` : `${pickerBag.length} cartas • compare HP, ataque, Energia e efeitos`}</Text></View><Ionicons name="chevron-forward" size={22} color={colors.accent} /></Pressable>
        ...
        {completed ? (
          <View style={[styles.resultPanel, { backgroundColor: colors.surface, borderColor: colors.yellow }]}>
            ...
            <View style={styles.actions}><Pressable style={[styles.accept, { backgroundColor: colors.yellow }]} onPress={rematch} disabled={working}><Text style={styles.acceptText}>REVANCHE</Text></Pressable><Pressable style={[styles.secondary, { borderColor: colors.accent }]} onPress={() => router.push(`/battle-summary/${battle.id}`)}><Ionicons name="bulb-outline" size={16} color={colors.accent}/><Text style={[styles.secondaryText, { color: colors.text }]}>POR QUE?</Text></Pressable><Pressable style={[styles.secondary, { borderColor: colors.border }]} onPress={() => router.push(`/battle-replay/${battle.id}`)}><Ionicons name="play-back" size={16} color={colors.accent}/><Text style={[styles.secondaryText, { color: colors.text }]}>REPLAY</Text></Pressable><Pressable style={[styles.secondary, { borderColor: colors.border }]} onPress={() => router.push('/battles')}><Text style={[styles.secondaryText, { color: colors.text }]}>BATTLE CENTER</Text></Pressable></View>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  timerPanel: { padding: 11, borderRadius: 16, borderWidth: 1, borderColor: 'transparent' }, turnStateRow:{flexDirection:'row',alignItems:'center',gap:9}, turnStateTitle:{fontSize:16,fontWeight:'900'}, roundLabel: { fontSize: 8, fontWeight: '900', letterSpacing: 1.1, marginTop:2 }, timer: { fontSize: 29, fontWeight: '900' }, timerHint: { fontSize: 9, textAlign: 'left', lineHeight: 14, marginTop: 7 }, rulesText: { maxWidth: 620, fontSize: 9, lineHeight: 14, fontWeight: '800', textAlign: 'center', marginTop: 7 },
