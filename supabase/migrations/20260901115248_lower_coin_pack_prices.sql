-- Lower coin booster prices while preserving diamond booster pricing.
-- Coin packs now use 75% of the undiscounted price with a 4k floor.
-- Diamond packs keep the existing 90% multiplier and 15-diamond floor.

update public.economy_policy
set coin_pack_ceiling=75000,
    notes=coalesce(notes,'{}'::jsonb) || jsonb_build_object(
      'boosterPriceMultiplier',0.75,
      'boosterCoinPriceMultiplier',0.75,
      'boosterDiamondPriceMultiplier',0.90,
      'boosterCoinPriceRelief','2026-09-01',
      'coinPackFloor',4000,
      'diamondPackFloor',15
    ),
    updated_at=now()
    ...
    select
      ...
      case
        when currency='diamonds' then
          ...
          greatest(
            4000::bigint,
            (round((undiscounted_price::numeric*0.75)/500.0)*500)::bigint
          )
      end::bigint as price
    from base_price
  )
