create or replace function private.get_my_missions_v2()
returns table(id text,title text,description text,cadence text,event_type text,target integer,reward_coins integer,reward_xp integer,reward_diamonds integer,action_route text,progress integer,claimed boolean,period_start date)
language plpgsql
security definer
set search_path to ''
as $function$
...
begin
  ...
  loop
    ...
    on conflict on constraint player_missions_v2_pkey do update
      ...
      where public.player_missions_v2.progress is distinct from excluded.progress;
  end loop;
