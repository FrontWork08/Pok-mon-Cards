-- Harden privileged Deck Builder stats behind a private helper and add covering indexes.

create index if not exists battles_forfeited_by_idx
  on public.battles(forfeited_by)
  where forfeited_by is not null;

create index if not exists guild_chat_messages_player_idx
  on public.guild_chat_messages(player_id,created_at desc);
...
revoke all on function private.get_my_deck_builder_page_v2_impl(integer,integer,text,text,text,text)
from public,anon;
grant execute on function private.get_my_deck_builder_page_v2_impl(integer,integer,text,text,text,text)
to authenticated;
...
set search_path=''
as $$
  select private.get_my_deck_builder_page_v2_impl(
    p_offset,p_limit,p_search,p_type_filter,p_rarity_filter,p_sort_mode
  );
$$;
