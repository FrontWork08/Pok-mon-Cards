...
-- Coin packs use 50% of the undiscounted price with a 2.5k floor and 50k ceiling.
-- Price-aware pull-quality thresholds are shifted down so lower prices do not reduce booster quality.

update public.economy_policy
set coin_pack_ceiling=50000,
    notes=coalesce(notes,'{}'::jsonb) || jsonb_build_object(
      'boosterPriceMultiplier',0.50,
      'boosterCoinPriceMultiplier',0.50,
      'boosterDiamondPriceMultiplier',0.90,
      'boosterCoinPriceRelief','2026-09-01-more-v2',
      'coinPackFloor',2500,
      'diamondPackFloor',15
    ),
    updated_at=now()
    ...
    select
      ...
      case
        when currency='diamonds' then
          ...
          greatest(
            2500::bigint,
            (round((undiscounted_price::numeric*0.50)/500.0)*500)::bigint
          )
      end::bigint as price
    from base_price
  )
      ...
      (
        case
          when public.rarity_tier(p_rarity)>=7 then
            case
              when (coalesce(p_currency,'coins')='coins' and coalesce(p_price,0)>=30000)
                or (coalesce(p_currency,'coins')='diamonds' and coalesce(p_price,0)>=60) then 1.30
              when (coalesce(p_currency,'coins')='coins' and coalesce(p_price,0)>=15000)
