...
begin
  ...
  select jsonb_build_object(
    ...
    'collection',jsonb_build_object(
      ...
      'trophies',coalesce((
        select jsonb_agg(jsonb_build_object(
          'id',i.id,'name',i.name,'description',i.description,'icon',i.icon,
          'rarity',i.rarity,'quantity',pei.quantity,'acquiredAt',pei.purchased_at
        ) order by
          case lower(i.rarity)
            when 'legendary' then 6 when 'master' then 5 when 'auction' then 5
            when 'luxury' then 4 when 'epic' then 3 when 'rare' then 2 else 1 end desc,
          pei.purchased_at desc,
          i.name
        )
        from public.player_economy_items pei
        join public.economy_store_items i on i.id=pei.item_id
        where pei.player_id=p.id and pei.quantity>0 and i.category='trophy'
      ),'[]'::jsonb)
    )
  ) into v_result
