...
function rarityTheme(rarity?: string | null): RarityTheme {
  ...
  if (value.includes('hyper') || value.includes('secret') || value.includes('special illustration') || value.includes('shiny ultra')) return { color: '#D9B24C', soft: '#2D240A', label: 'RARIDADE MÁXIMA', tier: 5 };
...
export function PackOpeningModal({ visible, pack, onClose, onPurchase, onFinished }: Props) {
  ...
  const bestPull = useMemo(() => {
    ...
    const pricedCards = cards.filter(
      (card) => card.marketPriceUsd != null && Number(card.marketPriceUsd) > 0,
    );
    if (pricedCards.length) {
      return pricedCards.reduce(
        (best, card) => Number(card.marketPriceUsd ?? 0) > Number(best.marketPriceUsd ?? 0) ? card : best,
        pricedCards[0],
      );
    }
    return cards.reduce(
      (best, card) => rarityScore(card) > rarityScore(best) ? card : best,
      cards[0],
    );
  }, [cards]);
  const pricedCardCount = useMemo(
    () => cards.filter((card) => card.marketPriceUsd != null && Number(card.marketPriceUsd) > 0).length,
    [cards],
  );
  const totalMarketValueUsd = useMemo(
    () => cards.reduce((sum, card) => sum + Math.max(0, Number(card.marketPriceUsd ?? 0)), 0),
    [cards],
  );
  const totalMarketValueLabel = pricedCardCount === 0
    ? 'Valor do booster: aguardando cotação'
    : pricedCardCount === cards.length
      ? `Valor do booster: ${formatUsd(totalMarketValueUsd)}`
      : `Valor parcial: ${formatUsd(totalMarketValueUsd)} • ${pricedCardCount}/${cards.length} cotadas`;
  const bestPullLabel = bestPull
    ? bestPull.marketPriceUsd != null && Number(bestPull.marketPriceUsd) > 0
      ? `Melhor pull: ${bestPull.name} • ${formatUsd(bestPull.marketPriceUsd)}`
      : `Melhor pull por raridade: ${bestPull.name}`
    : null;
  if (!pack) return null;
  ...
  return <Modal visible={visible} animationType="fade" transparent={false} onRequestClose={() => { if (stage !== 'opening') onClose(); }}>
    <View style={styles.container}>
      ...
      {(stage === 'sealed' || stage === 'opening') ? <View style={styles.openingStage}>
        ...
        {SPARKS.map((rotation) => <Animated.View pointerEvents="none" key={`os-${rotation}`} style={[styles.openingSpark, { backgroundColor: rotation % 36 === 0 ? '#fff' : '#D9B24C', opacity: openingSparkOpacity, transform: [{ rotate: `${rotation}deg` }, { translateY: openingSparkTravel }] }]} />)}
            ...
            <Animated.View style={[styles.flipScene, compact && styles.flipSceneCompact, { opacity: cardEnter, transform: [{ translateY: cardTranslateY }, { scale: combinedCardScale }] }]}>
              ...
              <Animated.View style={[styles.cardFace, styles.cardFront, { opacity: frontOpacity, borderColor: theme.color, transform: [{ scaleX: frontScaleX }] }]} pointerEvents="none">{(() => {
                ...
                <View style={styles.pullBadges}>
                  ...
                  {currentCard.wishlistHit ? <View style={styles.chaseBadge}><Ionicons name="star" size={11} color="#080B13" /><Text style={styles.chaseBadgeText}>CHASE HIT</Text></View> : null}
                </View>
...
const styles = StyleSheet.create({
  ...
  buyAgainButton:{minHeight:44,borderRadius:12,paddingHorizontal:13,backgroundColor:'#D9B24C',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6},
  buyAgainText:{color:'#080B13',fontSize:8,fontWeight:'900'},
  ...
  chaseBadge: { borderRadius:999, paddingHorizontal:9, paddingVertical:4, backgroundColor:'#D9B24C', flexDirection:'row', alignItems:'center', gap:4 },
  pullBadgeText: { color:'#fff', fontSize:8, fontWeight:'900', letterSpacing:.7 },
  chaseBadgeText: { color:'#080B13', fontSize:8, fontWeight:'900', letterSpacing:.5 },
